...
__int_str(
	...
	char b[], 
	int base, 
	bool isPlusSignIfNeeded,
	bool spaceSignIfNeeded, 
	int paddingNo, 
	bool justify, 
	bool zeroPadding)
	{
		char digit[MAX_DIGIT_COUNTER] = {0};
		memset(digit, 0, MAX_DIGIT_COUNTER);
		...
		if (i < 0){
			*p++ = '-';
			i *= -1; 
		} else if (isPlusSignIfNeeded){
			*p++ = '+';
		} else if (!isPlusSignIfNeeded && spaceSignIfNeeded){
			*p++ = ' ';
		}
